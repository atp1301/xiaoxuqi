...
### 同批完成：截图（`intro.md` 第 11 节第 14 项）

`docs/screenshots/` 下补齐了 4 张真实 `.png`：控制台（真实页面捕获）、
`--check-labs`、complex-web 报告全文、ExploitGym 官方 scorer 输出。
本轮**不覆盖**本文件开头那节的历史结论 —— 那里的
`[截图占位 …]` 描述的是旧机器交付时的状态，**已被这 4 张图取代**，
但该节按本文档"追加不覆盖历史"的惯例原样保留。
